...
type props = {
  ...
  windowsRefProp?: React.RefObject<HTMLDivElement | null>;
  pathRef?: React.RefObject<SVGGeometryElement | null>;
};

const OpeningDesktop = React.forwardRef<HTMLDivElement, props>(
  ({ sectionProgress, windowsRefProp, pathRef }, forwardedRef) => {
    const openingDesktopRef = useRef<HTMLDivElement | null>(null);
    const setOpeningDesktopRef = (el: HTMLDivElement | null) => {
      openingDesktopRef.current = el;
    ...
    const setWindowsRef = (el: HTMLDivElement | null) => {
      windowsRef.current = el;
      if (windowsRefProp) {
        (
          windowsRefProp as React.MutableRefObject<HTMLDivElement | null>
        ).current = el;
      }
    };
    const contentRef = useRef<HTMLDivElement>(null);
    const containerRef = useRef<HTMLDivElement>(null);
    const separateWindowRef = useRef<HTMLDivElement>(null);
    ...
    useEffect(() => {
      ...
      const t2 = setTimeout(() => setIsAnimating(true), 1000);
      return () => {
        clearTimeout(t);
        clearTimeout(t2);
      };
    ...
    useEffect(() => {
      ...
      <div class='reposition absolute top-0 left-0 w-full h-6 border-b border-[var(--shade-300)] bg-[var(--shade-900)] flex items-center justify-between pl-3 caption-small cursor-all-scroll'>
        ...
        <div class='flex items-center h-full cursor-default'>
          ...
          "<div class='absolute w-full left-0 top-6 bottom-0 pt-2 pb-3 px-3 overflow-scroll'>" +
          content +
          "</div>";
        ...
        const openSideWindow = () => {
          ...
          if (windowsRef.current && separateWindowRef.current) {
            windowsRef.current.style.gap = "16px";
            separateWindowRef.current.style.flex = "1";
            separateWindowRef.current.appendChild(modal);
          }
        };
        const closeSideWindow = () => {
          ...
          modal.style.top = "50%";
          modal.style.left = "50%";
          modal.style.transform = `translate(${dragOffset.x}px, ${dragOffset.y}px)`;
          modal.style.translate = "-50% -50%";
          if (
            windowsRef.current &&
            containerRef.current &&
            separateWindowRef.current
          ) {
            windowsRef.current.style.gap = "0";
            separateWindowRef.current.style.flex = "0";
        ...
        const appendModal = (immediate = false) => {
          setTimeout(() => {
            if (!separateWindowRef.current) return;
            separateWindowRef.current.appendChild(modal);
            // Open all windows in side window by default
            if (!immediate) {
              openSideWindow();
            }
          }, 600);
        ...
        return { openSideWindow, closeSideWindow, appendModal };
      };

      const openAboutMe = (immediate = false) => {
      ...
      shoutOuts.current.addEventListener("click", () => {
        ...
        const { appendModal } = createModal(title, content);
        appendModal();
      });
    ...
    return (
      <div
        className="window absolute flex w-full rounded-2xl"
        ref={setOpeningDesktopRef}
        style={{ height: "60px", overflow: "hidden" }}
      >
        <div
          className="absolute inset-0 z-10 rounded-2xl flex items-center justify-center bg-[var(--shade-900)] overflow-hidden pointer-events-none duration-300"
        ...
        >
          ...
          >
            <path
              d="M18.5 26L18.184 25.581C17.474 24.637 17.297 23.194 17.747 22M13.5 26L13.816 25.581C14.526 24.637 14.703 23.194 14.253 22M11 26H21"
              stroke="var(--shade-300)"
            />
            <path
              d="M18 6H14C10.72 6 9.081 6 7.919 6.814C7.48904 7.11507 7.11507 7.48904 6.814 7.919C6 9.08 6 10.72 6 14C6 17.28 6 18.919 6.814 20.081C7.11507 20.511 7.48904 20.8849 7.919 21.186C9.08 22 10.72 22 14 22H18C21.28 22 22.919 22 24.081 21.186C24.511 20.8849 24.8849 20.511 25.186 20.081C26 18.92 26 17.28 26 14C26 10.72 26 9.081 25.186 7.919C24.8849 7.48904 24.511 7.11507 24.081 6.814C22.92 6 21.28 6 18 6Z"
              stroke="var(--shade-300)"
            />
            <path d="M11 11H21" stroke="var(--shade-300)" />
            <path d="M11 14H21" stroke="var(--shade-300)" />
            <path d="M11 17H21" stroke="var(--shade-300)" />
          </svg>
        ...
        <div
          ...
          style={{
            height: introComplete
              ? sectionProgress
                ? sectionProgress < 1
                  ? `calc(100% + ${20 - sectionProgress * 20}vh)`
                  : "100%"
                : "calc(100% + 20vh)"
              : "calc(100% + 0vh)",
          }}
        >
          <div
            className="flex-1 relative w-full h-full flex flex-col text-[var(--shade-300)] rounded-[16px] overflow-hidden z-1 bg-[linear-gradient(to_bottom,var(--screen)_0%,var(--surface)_100%)]"
            ref={containerRef}
          >
            ...
            <div className="absolute z-2 top-0 left-0 w-full h-6 bg-[var(--surface)] flex items-center gap-4 pl-3 pr-2 caption-small">
        ...
        <div
          className="absolute z-100 left-0 -bottom-8 w-full h-[10vh] bg-[var(--shade-900)] duration-200"
          style={{
            opacity: introComplete
              ? sectionProgress
                ...
                : 1
              : 0,
            mask: "linear-gradient(to bottom, transparent 0%, rgba(0,0,0,0.5) 100%)",
          }}
        ></div>
      </div>
    );
