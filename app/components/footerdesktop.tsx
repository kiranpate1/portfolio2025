...
const FooterDesktop = ({ sectionProgress }: props) => {
  ...
  useEffect(() => {
    ...
    if (sectionProgress > Projects.length + 1.9 && !showLinks.current) {
      ...
      typingRef.current.querySelectorAll(".feature").forEach((a, i) => {
        setTimeout(() => {
    ...
    >
      <div className="absolute inset-0 flex justify-center items-center">
        <div className="absolute transform-3d perspective-distant md:scale-100 scale-50">
          <div
            ref={computerRef}
            className="computer w-[calc((50vh-32px)*1.253)] relative flex justify-center items-center transform-3d"
          >
            <svg
            ...
            <div
              className="absolute w-[79%] aspect-[405/304] translate-z-[-15px] bg-[var(--screen)]"
              style={{
                clipPath:
                ...
                <div className="relative h-[73px] w-full">
                  ...
                  <div className="absolute inset-0 flex flex-col gap-2">
                    {Array.from({ length: 10 }).map((_, index) => (
                      ...
                      >
                        <div className="h-1.75 w-full bg-[var(--screen)]"></div>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="flex justify-center gap-8 w-full caption-small text-[var(--shade-400)]">
                  <div className="feature opacity-0">designer</div>
                  <div className="feature opacity-0">developer</div>
                  <div className="feature opacity-0">motion designer</div>
                  <div className="feature opacity-0">artist</div>
                </div>
              </div>
              {/* <img
                ref={imageRef}
            ...
            <div
              className="absolute top-[50%] translate-y-[-50%] w-[112%] h-[7.35%] origin-bottom rotate-x-90 transform-3d"
