...
const Header = React.forwardRef<HTMLDivElement, props>(
  ({ height, scrollProgress, pathRef }, forwardedRef) => {
    ...
    useLayoutEffect(() => {
      ...
      if (!el) return;
      // Suppress transition on first appearance — prevents fade-in when scrollWindow visibility changes
...
/_/ |_/___/_/ |_/_/  |_/_/ |_/_/   /_/  |_|(_)`}
    ...
    const setTerminalRef = (el: HTMLDivElement | null) => {
    ...
    return (
      ...
      >
        <div
          ref={overlayRef}
          className="absolute inset-0 z-10 flex items-center justify-center rounded-2xl bg-[var(--shade-900)] overflow-hidden pointer-events-none"
        >
          <svg
            className="absolute"
            width="32"
          ...
          >
            ...
            />
            <path
              d="M22 21L30 16L22 11"
              stroke="var(--shade-300)"
              strokeLinejoin="round"
            />
            <path d="M12 26L20 6" stroke="var(--shade-300)" />
          </svg>
          ...
          >
            <path
              ref={pathRef}
              d="M16.5 0.5L43.5 0.5C52.337 0.5 59.5 7.66344 59.5 16.5V43.5C59.5 52.337 52.337 59.5 43.5 59.5H16.5C7.66344 59.5 0.5 52.337 0.5 43.5V16.5C0.5 7.66344 7.66344 0.5 16.5 0.5Z"
              stroke="var(--shade-300)"
        ...
        >
          {Array.from({ length: 3 }).map((_, index) => (
            <div
              key={index}
              className="w-full border-b border-b-[inherit]"
            ></div>
          ))}
        </div>
      </div>
